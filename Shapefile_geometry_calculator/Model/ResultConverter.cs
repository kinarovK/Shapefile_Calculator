...
namespace Shapefile_geometry_calculator.Model
{
    ...
    {
        public async Task<List<Result_Model>> ConvertPolygonResultAsync(List<Result_Model> results, Enum measureType)

        {
            if (results.Count == 0) return null;
            if (measureType.CompareTo(AreaMeasure.Sqrkilometer) == 0)
            {
                await CalculateConvertionAsync(results, 1000000);
            }
            else if (measureType.CompareTo(AreaMeasure.SqrMeter) == 0)
            {
                
                await CalculateConvertionAsync(results, 1);
            }
            else if (measureType.CompareTo(AreaMeasure.Hectar)== 0)
            {
                await CalculateConvertionAsync(results, 10000);
            }
            return results;
        }
        public async Task<List<Result_Model>> ConvertPolylineResultAsync(List<Result_Model> results, Enum measureType)
        {
            if (results.Count == 0) 
                return null;
            ...
            {
                await CalculateConvertionAsync(results, 1000);
            }
            return results;
        }

        private async Task<List<Result_Model>> CalculateConvertionAsync(List<Result_Model> results, int num)
        {
